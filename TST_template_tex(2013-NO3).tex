...
\hat{y}(i,j) = 
\begin{cases}
1, &\arg\underset{k \in \mathcal{K}}{\max}\,p_{a}^k(i,j)=y(i,j)\\
0, &otherwise
\end{cases}
\end{split}
\end{equation} 
...
\setlength\belowdisplayskip{8pt}
\begin{split}
p_{r}(i,j) = \mathcal{F}_{refine}(I, \mathcal{W}_r)\\
\end{split}
\end{equation}
...
\setlength\belowdisplayskip{8pt}
\begin{split}
p_{weighted}(i,j) &= p_{a}(i,j) \cdot p_{c}(i,j)\\
&+ (1-p_{a}(i,j)) \cdot p_{r}(i,j)\\
\end{split}
\end{equation}
...
\setlength\belowdisplayskip{8pt}
\begin{split}
\mathcal{L}_{refine}(y, p_{weighted}) = - \frac{1}{N}[\sum_{(i,j)\in I}log(p^{y(i,j)}_{weighted}(i,j))]\\
\end{split}
\end{equation}
