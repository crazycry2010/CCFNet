...
\begin{split}
p_{c}(i,j) &= \mathcal{F}_{coarse}(I, \mathcal{W}_c)\\
\end{split}
\end{equation} 
...
During the training process, the attention branch is supervised by a label map with 0/1 values, indicating  easy or difficult for the pixel in corresponding position. The attention branch is cascaded behind the coarse segmentation branch, so the label map can be generated by a comparison between the prediction of the coarse segmentation branch and the segmentation ground truth. 0 indicates the pixel is misclassified by the coarse segmentation branch, while 1 represents a correctly prediction. The 0/1 label map is used as the ground truth of the attention branch, supervising the attention branch to learn the segmentation difficulty of each pixel.
...
\setlength\belowdisplayskip{8pt}
\begin{split}
p_{a}(i,j) = \mathcal{F}_{attention}(I, \mathcal{W}_a)\\
\end{split}
\end{equation} 
...
\end{split}
\end{equation}
where $\hat{y}$ is the pixel-wise binary label of the input image, and 1 and 0 are the positive and negative labeled pixels. K is the number of instances
where β is the percentage of non-edge pixels in the image to account for skewness of sample numbers, 
